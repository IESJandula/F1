...
async function buscarPilotos(terminoBusqueda) {
  ...
  async function listarPilotos(resultado) {
    ...
    const pilotosContainer = document.getElementById('circuito');
  ...
  async function listarEquipos(equipos){
    ////////////empieza contenido//////
    var divEquipos = document.getElementById("circuito");
  ...
  async function listarCircuitos(circuitos){
    ///////inicia////////
    var divCircuitos = document.getElementById("circuito");
  ...
  window.addEventListener('load', async function () {
    ...
    const resultadosCircuito = await buscarCircuitos();                 ////////modificar por lo que toque se vaya a mostrar en la pagina
    listarCircuitos(resultadosCircuito);                                 ////////modificar por lo que toque se vaya a mostrar en la pagina
    ...
    searchInput.addEventListener('input', async function (event) {
      ...
      if (terminoBusqueda) {
        ...
        const resultadosCircuito = await buscarPilotos(terminoBusqueda);
        ...
        if (resultadosCircuito && resultadosCircuito.length > 0) {
          ...
          listarPilotos(resultadosCircuito);
        } else {
          ...
          if(resultadoEquipos && resultadoEquipos.length > 0){
            ...
            if( resultadoCircuitos && resultadoCircuitos.length > 0){
              ...
              const resultadosCircuito = await buscarPilotos();            ///////si no busca nada que muestr lo que toque en la pagina
              listarPilotos(resultadosCircuito);                          ///////si no busca nada que muestr lo que toque en la pagina
            }
          }
        }
      } else {
